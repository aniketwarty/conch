...
export const QuizPageDisplay = ({uid, studySetString, initialOptions}: QuizPageDisplayProps) => {
    ...
    async function updateOption(option_name: string, value: any) {
        setOptions({ ...options, [option_name]: value });
    }
        ...
        <div className="flex flex-col bg-slate-100 h-screen w-screen overflow-hidden">
            ...
            <div className="flex flex-col h-5/6 w-1/2 shadow-2xl m-auto p-5 items-center">
                ...
                <p className="text-2xl m-5 self-center">
                    {options["Number of Questions"]===-1 ? (studySet.terms.length + " questions"):(Math.max(studySet.terms.length, options["Number of Questions"]) + " questions")} 
                    {" • "}
                ...
                <div className="flex flex-row my-2">
                    ...
                    <Checkbox
                        ...
                        onChange={(e) => {
                            updateOption("True/False Questions", e.target.checked)
                        }}
                    />
                </div>
                <div className="flex flex-row my-2">
                    ...
                    <Checkbox
                        ...
                        onChange={(e) => {
                            updateOption("Multiple Choice Questions", e.target.checked)
                        }}
                    />
                </div>
                <div className="flex flex-row my-2">
                    ...
                    <Checkbox
                        ...
                        onChange={(e) => {
                            updateOption("Free Response Questions", e.target.checked)
                        }}
                    />
                </div>
                <Button className="mt-auto mb-10" colorScheme="blue" size="lg">
                    Generate quiz
                </Button>
            </div>
        </div>
